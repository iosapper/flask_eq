...
class KeshiForms(FlaskForm):
    kename = StringField('科室名称', validators=[DataRequired(), Length(1, 10)])
    submit = SubmitField('添加科室')

class IninfoForms(FlaskForm):
    prid = StringField('检验项目编号', validators=[DataRequired(), Length(1, 10)])
    smj = FileField('扫描件',validators=[FileAllowed(['jpg', 'png', 'gif']),DataRequired()])
